# This Makefile is for the RRD::Editor extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/RRD/Editor.pm]
#     AUTHOR => q[Doug Leith <dougleith@cpan.org>]
#     NAME => q[RRD::Editor]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/RRD/Editor.pm]
#     clean => { FILES=>q[RRD-Editor-*] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/RRD/Editor.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/RRD
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/RRD
...
TO_INST_PM = lib/RRD/Editor.pm

PM_TO_BLIB = lib/RRD/Editor.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/RRD/Editor.pm $(INST_MAN3DIR)/RRD::Editor.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         RRD-Editor' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.01_5' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/RRD/Editor.pm' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,01_5,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Portable, standalone (no need for RRDs.pm) tool to create and edit RRD files.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Doug Leith &lt;dougleith@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/RRD/Editor.pm blib/lib/RRD/Editor.pm 
