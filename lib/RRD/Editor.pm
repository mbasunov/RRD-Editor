...
sub _cookie {
    # Getting RRD file float cookie right is a little tricky because Perl rounds 8.642135E130 up to 8.6421500000000028e+130 on Intel 32 bit machines, 
    # and rounds to something else on 64 bit machines, and neither of these give the same bit sequence as C when Perl stores 8.642135E130.  Sigh ...  
    # If possible, use approach that avoids need for Inline module.  Ok so long as machine uses IEEE doubles (i.e. all modern machines) and either little-endian or big-endian byte order (i.e. most modern machines):
...
sub dump {
    ...
    $out.=sprintf "%s", "\t<step>".$rrd->{pdp_step}."</step> <!-- Seconds -->\n\t<lastupdate>".$rrd->{last_up}."</lastupdate> <!-- ".time2str("%Y-%m-%d %T %Z",$rrd->{last_up}). "-->\n\t";
    ...
    for ($i=0; $i<$rrd->{rra_cnt}; $i++) {
        ...
        $t = $rrd->{last_up} - $rrd->{last_up}%($rrd->{rra}[$i]->{pdp_cnt}*$rrd->{pdp_step}) -($rrd->{rra}[$i]->{row_cnt}-1)*$rrd->{rra}[$i]->{pdp_cnt}*$rrd->{pdp_step};
            for ($j=0; $j<$rrd->{rra}[$i]->{row_cnt}; $j++) {
                ...
                $out.=sprintf "\t%s", "<!-- ".time2str("%Y-%m-%d %T %Z",$t). " / $t --> <row>";
...
sub open {
    ...
    my $file_floatcookie_native_single=sprintf("%0.6e",_unpackd($self,substr($staticheader,12,PORTABLE_SINGLE_EL_SIZE),"native-single")); 
    my $file_floatcookie_portable_single=sprintf("%0.6e",_unpackd($self,substr($staticheader,12,PORTABLE_SINGLE_EL_SIZE),"portable-single")); 
    my $file_floatcookie_portable_double=sprintf("%0.6e",_unpackd($self,substr($staticheader,12,PORTABLE_DOUBLE_EL_SIZE),"portable-double")); 
    my $file_floatcookie_littleendian_single; 
    my $file_floatcookie_littleendian_double; 
    if ($PACK_LITTLE_ENDIAN_SUPPORT>0) {
        $file_floatcookie_littleendian_single=sprintf("%0.6e",_unpackd($self,substr($staticheader,12,PORTABLE_SINGLE_EL_SIZE),"littleendian-single")); 
        $file_floatcookie_littleendian_double=sprintf("%0.6e",_unpackd($self,substr($staticheader,12,PORTABLE_DOUBLE_EL_SIZE),"littleendian-double")); 
    }
...
=item * RRD files generated confirmed to be binary compatible with rrdtool on Intel 32 and 64 bit platforms.  Testing on other platforms needed, although should "just work".
