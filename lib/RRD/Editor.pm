...
sub _native_double {
   # try to figure out the long/double alignment needed for the RRDTOOL file format
    if ($Config{myarchname} =~ m/(sun|sparc|mips|irix|ppc|powerpc)/i && NATIVE_LONG_EL_SIZE==4) { # Only affects behaviour when writing new files from scratch,                                                                         
                                                                            # otherwise can figure out the right alignment to use when  
                                                                            # reading an existing file
        # Align long ints on 32 bit boundaries and doubles on 64 bit boundaries
...
Intel 386 32bit, Intel 686 32bit, AMD64/Intel x86 64bit, Itanium 64bit, Alpha 64bit, MIPS 32bit, MIPSel 32 bit, MIPS 64bit, PowerPC 32bit, ARM 926EJ-S v5l (32bit, ABI/mixed-endian floats), SPARC 32bit, SPARC 64bit, SH4
...
For more information on RRD::Editor portability testing, see L<http://www.glmonitoring.com/rrdeditor/>.  If your platform is not listed, there is a good chance things will "just work" but double checking that RRDTOOL can read the C<native-double> format RRD files generated by RRD::Editor, and vice-versa, would be a good idea if that's important to you.
